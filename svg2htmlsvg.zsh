...
typeset -gr SCRIPT_VERSION="1.0.2"
...
# Load zsh/datetime module at script level (once, not per-call).
# Plain zmodload (no -F) for zsh 5.0+ compatibility; -F feature selection
# was unreliable on versions prior to ~5.7.
zmodload zsh/datetime 2>/dev/null
typeset -gi _HAS_STRFTIME=$(( $+builtins[strftime] ))
...
_log() {
    ...
    # Generate timestamp using the zsh/datetime builtin if available,
    # otherwise fall back to the external date(1) command.
    local timestamp
    if (( _HAS_STRFTIME )); then
        strftime -s timestamp '%Y-%m-%d %H:%M:%S' "$EPOCHSECONDS"
    else
        timestamp=$(date '+%Y-%m-%d %H:%M:%S' 2>/dev/null) || timestamp='----'
    fi
...
parse_arguments() {
    ...
    #
    # NOTE: -F (fail on unknown options) is omitted intentionally —
    # it requires zsh >= 5.8.  Unknown options are caught manually
    ...
    # Manual unknown-option check (replaces zparseopts -F for 5.0+ compat).
    # After -D removes known options, anything starting with - that remains
    # in $@ is unrecognized.
    local _arg
    for _arg in "$@"; do
        if [[ "$_arg" == -?* ]]; then
